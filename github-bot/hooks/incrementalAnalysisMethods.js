...
const incrementalAnalysisMethods = () => {
  ...
  const incremental_calculateRiskScore = (pullRequest, riskScoreSettings, qualityGateMetrics) => {
    ...
    const risk_value = parseFloat((((riskScoreCoefficients.page_rank_score_coefficient * pullRequest.analysis.current_page_rank_result.category + 
  ...
  return {incremental_calculateCurrentBugFrequencies, incremental_calculateCurrentCoChangeFiles, incremental_calculateCurrentCodeChurn, incremental_updateBranch, incremental_calculateRiskScore, incremental_calculateMergeRate, incremental_calculate_pagerank, incremental_generate_save_impact_graph, calculateMetricCategory};
}
