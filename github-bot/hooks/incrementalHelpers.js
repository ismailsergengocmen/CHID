...
const incrementalHelpers = () => {
  ...
  const getFileNameFromPath = (path) => {
	...
	const buildComment = (pullRequest, settings, impactGraphUrl, repoID) => {
	...
	const addComment = async (context, pullRequest, settings, impactGraphUrl) => {
		const repoID = context.payload.repository.id;
		const {owner, repo} = context.issue();
		const commentBody = buildComment(pullRequest, settings, impactGraphUrl, repoID);
